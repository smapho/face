...
2. トップページ → 「出退勤を打刻する」でカメラに顔を映し、「出社」「退社」「中抜け」「戻り」いずれかのボタンを押して打刻
3. トップページ → 「出退勤一覧を見る」で各従業員の現在の状態と打刻履歴（日付フィルタ付き）を確認

既存のSupabaseプロジェクトに `sql/schema.sql` を適用済みの場合、`sql/002_add_break_types.sql` をSQL Editorで実行して「中抜け」「戻り」の記録を有効にしてください。
